# percorrer toda a nossa base de dados
# para cada item 
    # verificar se o bairro já existe em uma aba, se não existir, criar aquela aba
    # copiar os valores daquela linha e colcoar na aba do bairro correspondente

from openpyxl import load_workbook
from copy import copy



def criar_aba(bairro, arquivo_bairros, estilos_cabecalho):
    if bairro not in arquivo_bairros.sheetnames:
        # criação de uma nova aba
        arquivo_bairros.create_sheet(bairro)

        nova_aba = arquivo_bairros[bairro]
        nova_aba["A1"].value = "Data de Nascimento"
        nova_aba["B1"].value = "Pessoa"
        nova_aba["C1"].value = "Bairro"
        nova_aba["A1"]._style = estilos_cabecalho
        nova_aba["B1"]._style = estilos_cabecalho
        nova_aba["C1"]._style = estilos_cabecalho


def transferir_informacoes_aba(aba_origem, aba_destino, linha_origem):
    linha_destino = aba_destino.max_row + 1
    for coluna in range(1, 4):
        celular_origem = aba_origem.cell(row=linha_origem, column=coluna)
        celular_destino = aba_destino.cell(row=linha_destino, column=coluna)

        celular_destino.value = celular_origem.value

        celular_destino._style = copy(celular_origem._style)

arquivo_bairros = load_workbook("Bairros.xlsx")

aba_base_dados = arquivo_bairros["Base de Dados"]

ultima_Linha = aba_base_dados.max_row
print(ultima_Linha)

estilos_cabecalho = copy(aba_base_dados["A1"]._style)

for linha in range(2, ultima_Linha + 1):
    bairro = aba_base_dados.cell(row=linha, column=3).value

    if not bairro:
        break

    # criar uma aba pro bairro
    criar_aba(bairro, arquivo_bairros, estilos_cabecalho)

    # transferir informacoes para a aba
    aba_destino = arquivo_bairros[bairro]
    transferir_informacoes_aba(aba_base_dados, aba_destino, linha)

arquivo_bairros.save("Bairros.xlsx")